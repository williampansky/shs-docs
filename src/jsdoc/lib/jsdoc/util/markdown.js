...
/**
 * Retrieve a function that accepts a single parameter containing 
 * Markdown source. The function uses the specified parser to transform 
 * the Markdown source to HTML, then returns the HTML as a string.
 *
 * @private
 * @param {String} parserName The name of the selected parser.
 * @param {Object} [conf] Configuration for the selected parser, if any.
 * @returns {Function} A function that accepts Markdown source, 
 * feeds it to the selected parser, and returns the resulting HTML.
 */
function getParseFunction(parserName, conf) {
    ...
    if (parserName === parserNames.marked) {
        ...
        // Marked generates an "id" attribute for 
        // headers; this custom renderer suppresses it
